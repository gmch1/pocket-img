...
- 页面顶部显示非阻塞上传进度和结果提示。
- 支持最近 7 天和全部媒体两个范围。
- 图片和视频按 `Asia/Shanghai` 日期倒序展示。
- 支持缩略图/视频时间线、完整媒体预览和公开链接复制。
- 支持单张永久删除、长按多选、桌面空白区域拖拽框选和批量永久删除。
- 通过按用户空间隔离的 SSE 实时感知上传、删除、缩略图完成和过期清理，并在浏览器重新可见或长连接异常时低频校验当前列表。
- 管理员可创建用户空间，新 Token 只显示一次。

### Go 图床后端

- 前端静态资源、SQLite、时区数据和 WebP 编解码能力内嵌在同一个二进制中。
- 支持 PNG、JPEG、GIF、WebP，以及无音频单 H.264 视频轨的 MP4 上传，并校验真实格式、文件大小和像素数。
...
- 支持撤销、复制截图、贴图置顶和上传图片。
- 支持 `F2` 选区录制无音频视频、时间轴裁剪、导出 H.264 MP4，并由用户选择复制文件或上传。
- 未配置有效的服务地址和 Token 时隐藏上传按钮，并禁用回车上传。
- 上传成功后自动复制公开 URL。
- 配置保存在固定应用数据目录，不访问 macOS 钥匙串。
- 设置支持跟随系统、简体中文和 English，菜单、截图交互与错误提示会立即切换语言。
