...
- 后端在上传大小、像素上限、有界队列和单 worker 之外，按稳定空间 ID 执行内存滚动窗口限流：上传与原图各 500 次/小时，单张缩略图 5 次/分钟且 10 次/小时，空间缩略图总量 2000 次/小时；上传并发为每空间 2 个。登录使用来源、全局和有效空间三层令牌桶。超限只返回 `429` 与 `Retry-After`，不持久化计数。
