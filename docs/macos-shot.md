...
- 服务地址、Token 和快捷键保存在当前 macOS 用户的应用偏好中，不访问钥匙串；Session Cookie 只保存在当前进程。
...
首次截图时，macOS 会要求“屏幕与系统音频录制”权限。这个名称是 macOS 对屏幕捕获权限类别的系统文案；客户端只在按下快捷键时获取一张静态画面，捕获配置明确关闭音频，也不会创建持续共享流。授权后若应用被系统退出，请重新打开一次。功能键行为受系统键盘设置影响；部分 Mac 键盘需要按 `Fn-F2` 才会发送标准 F2。

从 `0.4.1` 升级到 `0.4.2` 时需要重新填写一次服务地址和 Token，因为旧版数据位于已撤销的沙盒/钥匙串存储中；从 `0.4.2` 起直接替换 `.app` 不会再清空这些设置。
