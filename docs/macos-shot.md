...
- 菜单栏常驻，不显示 Dock 图标。
- 应用程序包使用独立的 PocketIMG Shot 图标，便于在 Finder、应用程序目录和系统设置中识别；菜单栏继续使用符合 macOS 习惯的单色图标。
...
- 文字输入态使用 18% 黑色轻遮罩和系统强调色焦点效果，placeholder、输入文字和最终标注统一使用当前颜色，并按字体行高垂直居中。
- 方框和箭头默认使用 2 点细线；选中后可用鼠标滚轮调节线宽，选中 `A` 后滚轮调节字号；浮层会短暂显示当前数值，后续截图与应用重启后继续沿用。
