...
- 支持多显示器区域选择及 Retina 像素比例；首次截取某块显示器时使用 macOS 系统内容选择器授权，本次运行内会复用授权结果。
- 支持固定红色矩形和红色箭头。
...
首次截取某块显示器时，macOS 会显示自己的内容选择器，请选择鼠标所在的显示器。客户端只获取单张静态画面，并在捕获配置中明确关闭音频；本次运行内再次截取同一显示器不会重复选择。功能键行为受系统键盘设置影响；部分 Mac 键盘需要按 `Fn-F2` 才会发送标准 F2。
